...
/*object Enrichment {
  def enrich(transactionRaw: TransactionRaw): ZIO[Enrichment, Any, IO[EnrichmentError, TransactionEnriched]] =
    ZIO.serviceWith[Enrichment](_.enrich(transactionRaw))
}*/
...
object Enrichment {
  def enrich(transactionRaw: TransactionRaw): ZIO[Enrichment, EnrichmentError, TransactionEnriched] =
    ...
    } yield TransactionEnriched(a.userId, a.country, a.amount)
}
